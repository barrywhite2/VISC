...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "VISC.conf"));
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("VISC Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  VISC-cli [options] <command> [params]  " + _("Send command to VISC Core") + "\n" +
                        "  VISC-cli [options] help                " + _("List commands") + "\n" +
                        "  VISC-cli [options] help <command>      " + _("Get help for a command") + "\n";
